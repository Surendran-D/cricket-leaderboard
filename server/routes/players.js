...
router.get('/leaderboard/batting', (req, res) => {
    try {
        const leaderboard = getBattingLeaderboard();
...
router.get('/leaderboard/bowling', (req, res) => {
    try {
        const leaderboard = getBowlingLeaderboard();
        res.json(leaderboard);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
router.get('/:id', (req, res) => {
    try {
        const player = getPlayerWithStats(parseInt(req.params.id));
        if (!player) {
...
router.post('/', (req, res) => {
    try {
        ...
        const result = createPlayer(name.trim());
...
router.post('/:id/image', upload.single('image'), (req, res) => {
    try {
        ...
        updatePlayerImage(playerId, imagePath);
