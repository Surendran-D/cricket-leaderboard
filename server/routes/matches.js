...
router.get('/:id/stats', async (req, res) => {
	try {
		...
		const battingStats = await getBattingStatsByMatch(matchId);
		const bowlingStats = await getBowlingStatsByMatch(matchId);
		res.json({
			match,
...
router.post('/', async (req, res) => {
	try {
		...
		// Create match
		const matchResult = await createMatch(match_date, match_name || null);
...
router.put('/:id', async (req, res) => {
	try {
		...
		// Update match
		await updateMatch(matchId, match_date, match_name || null);

		// Delete existing stats
		await deleteBattingStatsByMatch(matchId);
		await deleteBowlingStatsByMatch(matchId);
